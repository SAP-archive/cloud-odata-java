package com.sap.core.odata.processor.jpa.util;

public interface MockData {
	
	public static String ENTITY_NAME_1 = "SalesOrderHeader";
	public static String ENTITY_NAME_2 = "LineItems";
	public static String ENTITY_ID_NAME_1 = "soID";
	public static String ENTITY_ID_NAME_2 = "liID";
	public static Class<?> ENTITY_ID_TYPE = java.lang.Long.class;
	public static String NAME_SPACE ="salesorderprocessing";
	public static String ENTITY_CONTAINER_NAME = "salesorderprocessingContainer";

}
