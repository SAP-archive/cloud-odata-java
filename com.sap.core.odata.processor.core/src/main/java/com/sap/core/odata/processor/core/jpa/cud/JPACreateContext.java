...
public class JPACreateContext {
	...
	private HashMap<String, String> jpaEmbeddableKeyAttributeMap = null;
	...
	public JPACreateContext() {
		...
		jpaEmbeddableKeyAttributeMap = new HashMap<String, String>();
	}
		...
		try {
			for (String propertyName : structuralType.getPropertyNames()) {
				...
				try {
					if (property.getType().getKind().equals(EdmTypeKind.COMPLEX)) {
								for(EntityType<?> entity:metamodel.getEntities())
								{
									...
									{
										Attribute<?,?> attribute = entity.getAttribute(property.getMapping().getInternalName());
										propertyClass = attribute.getJavaType();
										try {
											jpaComplexObjectMap.put(property.getMapping().getInternalName(), propertyClass.newInstance());
										} catch (InstantiationException e) {
											throw ODataJPARuntimeException
											.throwException(ODataJPARuntimeException.GENERAL
													.addContent(e.getMessage()), e);
				...
				if (nameParts.length > 1) {
					jpaEmbeddableKeyAttributeMap.put(propertyName, property.getMapping().getInternalName());
	...
	public final Object parse2JPAEntityValueMap(
		...
		try {
			for (String key : setters.keySet()) {
				...
				if (method != null) {
					if (property.getType().getKind().equals(EdmTypeKind.COMPLEX)) {
						Object complexObject = jpaComplexObjectMap.get(property.getMapping().getInternalName());
						parse2JPAEntityValueMap(complexObject, ((EdmComplexType)property.getType()), 
								(Map<String, Object>)propertyValue, property.getMapping().getInternalName());
